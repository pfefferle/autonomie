	<header class="entry-header">
		...
		<?php if ( method_exists('Kind_Taxonomy', 'get_icon')) : ?>
			<div class="entry-meta post-kind">
				<?php
					$kind_slug = get_post_kind_slug( get_the_ID() );
					$kind = get_post_kind( get_the_ID() );
					echo Kind_Taxonomy::get_icon($kind_slug);
				?>
				<a class="kind <?php echo Kind_Taxonomy::post_class(); ?>" href="<?php echo esc_url(Kind_Taxonomy::get_post_kind_link($kind)); ?>">
					<?php echo Kind_Taxonomy::get_post_kind_string($kind); ?>
				</a>
			</div>
		<?php else : ?>
