...
function autonomie_get_search_form( $form ) {
	$form = preg_replace( '/<search><form/i', '<form itemprop="potentialAction" itemscope itemtype="https://schema.org/SearchAction"', $form );
	$form = preg_replace( '/<\/form></search>/i', '<meta itemprop="target" content="' . home_url( '/?s={s} ' ) . '"/></form>', $form );
