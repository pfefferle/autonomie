 ...
 * @return string
 */
function zenpress_comment_input_required($fields)
{
...
add_filter("comment_form_default_fields", "zenpress_comment_input_required");
...
if ( version_compare( $GLOBALS['wp_version'], '3.9', '<' ) ) {
  add_filter( 'img_caption_shortcode', 'zenpress_html5_caption', 10, 3 );
}
 ...
 */
function zenpress_html5_caption($empty, $attr, $content)
{
  extract(shortcode_atts(array(
    'id'      => '',
    'align'   => 'alignnone',
    'width'   => zenpress_content_width(),
    'caption' => ''
  ), $attr));
...
if ( version_compare( $GLOBALS['wp_version'], '3.6', '<' ) ) {
  add_filter("comment_form_default_fields", "zenpress_comment_input_types");
  add_filter("get_search_form", "zenpress_search_form_input_type");
}
 ...
 * @return string
 */
function zenpress_comment_input_types($fields)
{
 ...
 * @return string
 */
function zenpress_search_form_input_type($form)
{
  return preg_replace('/"text"/', '"search" placeholder="'.__('Search here&hellip;', 'zenpress').'"', $form);
}
 ...
 * @return string
 */
function zenpress_comment_field_input_type($field)
{
  return preg_replace('/<textarea/', '<textarea required', $field);
}
add_filter("comment_form_field_comment", "zenpress_comment_field_input_type");
